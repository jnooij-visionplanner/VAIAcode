...
function formatProcessType(process: ServerProcessDiagnosticsEntry): string {
  ...
  if (/\b(codex|claude|opencode|cursor|copilot)\b/i.test(process.command)) return "Agent";
  return "Process";
}
          ...
          if (!result.signaled) {
            ...
            if (isStaleProcessSignalMessage(message)) {
              toastManager.add({
                ...
                description:
                  "The process is not a child of the Vaia Server. It might already have exited.",
              });
              return;
            }
