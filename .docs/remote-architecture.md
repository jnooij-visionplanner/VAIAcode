...
This document describes the target architecture for first-class remote environments in Vaia Code.
...
- Keep the Vaia server as the execution boundary.
...
Vaia already has a clean runtime boundary: the client talks to a Vaia server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Vaia server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Vaia server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Vaia runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Vaia server instance.
...
How does the client speak WebSocket to a Vaia server?
...
- best fit for hosted or self-managed remote Vaia deployments
...
For Vaia, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
- reach a machine
- probe it
- launch or reuse a remote Vaia server
...
How does a Vaia server come to exist on the target machine?
...
The user or operator already runs Vaia on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Vaia:
...
The recommended Vaia flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Vaia availability.
3. Desktop launches or reuses a remote Vaia server.
...
This is the inverse of remote launch: a local Vaia server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Vaia server might be reached through direct `wss`.
...
Vaia already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Vaia should not copy that part.

Vaia already has the right runtime boundary:

- one Vaia server per environment
- ordinary HTTP/WebSocket between client and environment

So Vaia should borrow Zed's launch discipline, not its transport protocol.
