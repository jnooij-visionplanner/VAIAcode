...
Define the long-term server auth architecture for Vaia Code before first-class remote environments ship.
...
Every privileged surface of the Vaia server must go through the same auth policy engine:
...
DesktopMain -> Tunnel : publish local Vaia endpoint
