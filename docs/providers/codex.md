...
This guide is for people who want to use more than one Codex account in Vaia Code.
...
In Vaia Code Settings, name it something obvious:
...
In Vaia Code Settings, add another Codex provider:
...
Vaia Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Vaia Code treats it
